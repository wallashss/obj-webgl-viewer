...
function Fly()
{
	...
	{
        if(epsilonEqual(state.yawIntensity, 0.0, EPSILON) && epsilonEqual(state.pitchIntensity, 0.0, EPSILON) &&
           epsilonEqual(state.forward, 0.0, EPSILON) && epsilonEqual(state.backward, 0.0, EPSILON) &&
           epsilonEqual(state.left, 0.0, EPSILON) && epsilonEqual(state.right, 0.0, EPSILON) &&
           epsilonEqual(state.up, 0.0, EPSILON) && epsilonEqual(state.down, 0.0, EPSILON))
		{
			return false;
		}
...
//                 viewMatrix = yawRot * viewMatrix;
				...
				mat4.rotate(yawRot, yawRot, radians(state.yawIntensity * state.angularVelocity * dt), up);
